...
import { darkPalette, lightPalette, darkShadows, lightShadows } from '@/design/tokens';

describe('normalizeSystem', () => {
  type Cases = Array<{ name: string; input: SystemScheme; expected: 'light' | 'dark' | null }>;
  const cases: Cases = [
...
describe('resolveTheme', () => {
  ...
  const cases: Cases = [
    ...
    { name: 'undefined pref + dark system → dark', pref: undefined, system: 'dark', expectScheme: 'dark' },
    { name: 'undefined pref + light system → light', pref: undefined, system: 'light', expectScheme: 'light' },
    // System = unspecified or null is treated as dark (dark-first identity).
    { name: 'undefined pref + unspecified system → dark (dark-first)', pref: undefined, system: 'unspecified', expectScheme: 'dark' },
    { name: 'undefined pref + null system → dark (dark-first)', pref: undefined, system: null, expectScheme: 'dark' },

    // Explicit 'system' preference behaves like undefined fallback.
    { name: 'system pref + light system → light', pref: 'system', system: 'light', expectScheme: 'light' },
    { name: 'system pref + dark system → dark', pref: 'system', system: 'dark', expectScheme: 'dark' },

    // Explicit overrides win regardless of system.
    { name: 'dark pref + light system → dark', pref: 'dark', system: 'light', expectScheme: 'dark' },
    { name: 'light pref + dark system → light', pref: 'light', system: 'dark', expectScheme: 'light' },
    { name: 'light pref + null system → light', pref: 'light', system: null, expectScheme: 'light' },
  ];
  ...
  for (const c of cases) {
    it(c.name, () => {
      ...
      expect(theme.colors).toBe(c.expectScheme === 'dark' ? darkPalette : lightPalette);
      // shadow(name) should resolve from the matching palette's shadow table.
      expect(theme.shadow('sm')).toBe(c.expectScheme === 'dark' ? darkShadows.sm : lightShadows.sm);
      expect(theme.shadow('glowAccent')).toBe(
        c.expectScheme === 'dark' ? darkShadows.glowAccent : lightShadows.glowAccent,
      );
    });
  }
});