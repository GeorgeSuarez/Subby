...
/** Web has no haptics — no-op instead of throwing. */
function onNative(fn: () => Promise<void>): Promise<void> {
...
export function impactLight(): Promise<void> {
  return onNative(() => Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light));
}

/** Soft impact — used for subtle chip / segmented selections. */
export function impactSoft(): Promise<void> {
  return onNative(() => Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Soft));
}

/** Rigid impact — used for snapping / pull-to-refresh style moments. */
export function impactRigid(): Promise<void> {
  return onNative(() => Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Rigid));
}
...
export function selection(): Promise<void> {
  return onNative(() => Haptics.selectionAsync());
}
