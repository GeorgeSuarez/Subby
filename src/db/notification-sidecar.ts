...
export async function getAllNotificationIds(): Promise<Record<string, string>> {
  ...
  const rows = await db.getAllAsync<{ subscription_id: string; notification_id: string }>(
    'SELECT subscription_id, notification_id FROM notification_map;',
  );
  return Object.fromEntries(rows.map((r) => [r.subscription_id, r.notification_id]));
}

/** Upsert the scheduled-notification id for a subscription. */
export async function setNotificationId(subscriptionId: string, notificationId: string): Promise<void> {
  const db = await getDatabase();
  await db.runAsync(
...
export async function deleteNotificationId(subscriptionId: string): Promise<void> {
  const db = await getDatabase();
  await db.runAsync('DELETE FROM notification_map WHERE subscription_id = ?;', subscriptionId);
}
