...
import { TextField, type TextFieldProps as TextFieldProps_in } from '@/features/add-subscription/components/TextField';

type TextRef = ComponentProps<typeof TextField>['ref'] extends React.Ref<infer T> ? T : never;

const DECIMAL_RE = /^\d*\.?\d{0,2}$/;

export interface AmountInputProps extends Omit<TextFieldProps_in, 'keyboardType' | 'leading' | 'trailing' | 'onChangeText'> {
  currency: CurrencyCode;
  value: string;
  onChangeText: (raw: string) => void;
}

export const AmountInput = forwardRef<TextRef, AmountInputProps>(function AmountInput(
  { currency, value, onChangeText, ...rest },
  ref,
) {
